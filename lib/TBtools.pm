...
@EXPORT     =  qw(parse_reference
                  ...
                  specificator_beijing_easy
                  specificator_coll_easy
                  specificator_coll_branch
                  specificator_homolka
                 );
...
sub multi_fasta { # creates a multi fasta file from input.
   ...
   open(FASTA, ">$AMEND_OUT/$phylo_fasta") || print $logprint "<ERROR>\t",timer(),"\tCan't create $phylo_fasta: TBtools line:", __LINE__ , " \n";
...
sub fastaids { # create output of plain fasta IDs.
   my $logprint   =  shift;
   my $AMEND_OUT  =  shift;
   my $infasta    =  shift;
   my $outfasta   =  strip($infasta,(".plainIDs.fasta") );
   open(IN,"$AMEND_OUT/$infasta")      ||	die print $logprint "<ERROR>\t",timer(),"\tCan't open $infasta: TBtools line: ", __LINE__ , " \n";
   open(OUT,">$AMEND_OUT/$outfasta")   ||	die print $logprint "<ERROR>\t",timer(),"\tCan't create $outfasta: TBtools line: ", __LINE__ , " \n";
   while(<IN>) {
      my $line       =  $_;
      $line          =~ s/\015?\012?$//;
      next if($line) =~ /^#/;
...
sub parse_reference { # parse a reference genome and save all posiitions as a unique key.
   ...
   my $ref_hash   =  shift;
   my $position   =  1;
   open(IN,"$VAR_dir/$ref") || die print $logprint "<ERROR>\t",timer(),"\tCan't open $ref: TBtools line: ", __LINE__ , " \n";
   while(<IN>) {
...
sub parse_mpile { # parse .mpileup file.
   ...
   unlink("$POS_OUT/$output\.tmp") || print $logprint "<WARN>\t",timer(),"\tCan't delete $output\.tmp: TBtools line:", __LINE__ , " \n";
...
sub parse_position_table { # parse a position table.
   ...
   open(IN,"$POS_OUT/$position_file") || die print $logprint "<ERROR>\t",timer(),"\tCan't open $position_file: TBtools line: ", __LINE__ , " \n";
   <IN>;
   while(<IN>) {
      my $line          =  $_;
      $line             =~ s/\015?\012?$//; # this will take care of different line break characters.
      ...
      if($position_stats && $insertion_index == 0) {
         ...
         my $N          =  $line[5];
         my $GAP        =  $line[6];
         ### feverse read nucleotides count
         my $a          =  $line[7];
         my $c          =  $line[8];
         my $g          =  $line[9];
         my $t          =  $line[10];
...
sub parse_annotation { # parse an annotation file.
   ...
   my $annotation    =  shift;
   my $refg          =  shift;
   open(IN,"$VAR_dir/$refg") || die print $logprint "<ERROR>\t",timer(),"\tCan't open $refg: TBtools line: ", __LINE__ , " \n";
   while(<IN>) {
      chomp;
      my $line       =  $_;
      $line          =~ s/\015?\012?$//; # this will take care of different line break characters
...
sub parse_amend_table { # parse an amended joint variant table.
   ...
   open(IN,"$AMEND_OUT/$phylo_file") || die print $logprint "<ERROR>\t",timer(),"\tCan't open $phylo_file: TBtools line: ", __LINE__ , " \n";
...
sub prepare_stats { # print a statistics file.
   my $statistics          =  shift;
...
sub print_joint_table_scaffold { # print the scaffold of a joint SNP table.
   my $logprint      =  shift;
   my $JOIN_OUT      =  shift;
   my $join_file     =  shift;
   my $var_positions =  shift;
   ...
   open(OUT, ">$JOIN_OUT/$join_file") || die print $logprint "<ERROR>\t",timer(),"\tCan't create $join_file: TBtools line: ", __LINE__ , " \n";
...
sub print_joint_table { # print a joint SNP table.
   ...
   open(IN, "$JOIN_OUT/$join_file") || die print $logprint "<ERROR>\t",timer(),"\tCan't open $join_file: TBtools line: ", __LINE__ , " \n";
   open(OUT, ">$JOIN_OUT/$tmp")     || die print $logprint "<ERROR>\t",timer(),"\tCan't create $tmp: TBtools line: ", __LINE__ , " \n";
   my $header        =  <IN>;
   my $subheader     =  <IN>;
   print OUT $header;
   print OUT $subheader;
   ...
   close(OUT);
   close(IN);
   move("$JOIN_OUT/$tmp","$JOIN_OUT/$join_file") || die print $logprint "<ERROR>\t",timer(),"\tmove failed: TBtools line: ", __LINE__ , " \n";
   unlink("$JOIN_OUT/$tmp");
}
...
sub strip { # strip the filenme of an input filename.
   my $filename         =  shift;
   my $extension        =  shift;
   $filename            =~ s/\.\w+$//;
   if($extension) {
...
sub cat { # simple cat/copy function.
   my $logprint   =  shift;
   my $in         =  shift;
   my $out        =  shift;
   open(OUT,">>$out");
   open(IN,"$in") || die print $logprint "<WARN>\t",timer(),"\tCan't open $in: TBtools line: ", __LINE__ , " \n";
   while(<IN>) {
      print OUT $_;
   }
   close(IN);
...
sub log2 { # return the log to the base 2.
   my $n = shift;
   return(log($n)/log(2));
}
...
sub amend_joint_table { # amend a joint variant Table.
   my $logprint         =  shift;
   my $JOIN_OUT         =  shift;
   my $AMEND_OUT        =  shift;
   my $cats             =  shift;
   ...
   open(IN,"$JOIN_OUT/$join_file")        || die print $logprint "<ERROR>\t",timer(),"\t Can't open file $join_file: TBtools line: ", __LINE__ , " \n";
   open(OUT,">$AMEND_OUT/$amended_file")  || die print $logprint "<ERROR>\t",timer(),"\t Can't create file $amended_file: TBtools line: ", __LINE__ , " \n";
   open(OUTER,">$AMEND_OUT/$phylo_file")  || die print $logprint "<ERROR>\t",timer(),"\t Can't create file $phylo_file: TBtools line: ", __LINE__ , " \n";
   ...
   while(<IN>) {
      ...
      if ($main_type eq "Ins") {
         my $ins_counter   =  0;
         $ins_counter = $type_hash->{"Ins"};
         $perc_unambigous  =  sprintf("%.4f", $unambigous_tmp / $ins_counter * 100);
      }
      else {
...
sub filter_wlength { # filter for possible biased SNPs in a given window.
   my $logprint         =  shift;
   my $AMEND_OUT        =  shift;
   my $phylo_file       =  shift;
   my $window           =  shift;
   ...
   open(IN,"$AMEND_OUT/$phylo_file")            || die print $logprint "<ERROR>\t",timer(),"\t Can't open $phylo_file: TBtools line: ", __LINE__ , " \n";
   open(OUT, ">$AMEND_OUT/$window_file")        || die print $logprint "<ERROR>\t",timer(),"\t Can't create $window_file: TBtools line: ", __LINE__ , " \n";
   open(OUTER, ">$AMEND_OUT/$window_filtered")  || die print $logprint "<ERROR>\t",timer(),"\t Can't create $window_filtered: TBtools line: ", __LINE__ , " \n";
...
sub build_matrix { # build a strain matrix.
   my $GROUPS_OUT          =  shift;
...
sub call_variants { # call variants.
   my $logprint         =  shift;
   my $position_table   =  shift;
   my $variants         =  shift;
   my $statistics       =  shift;
      ...
      foreach my $insertion_index (keys %{$position_table->{$pos}}) {
         ...
         unless($type eq "Ins") {
            ...
            if($cov > 0) {
               ...
               # statistics unambiguous coverage.
               if($unambiguous_base_call == 1) {
                  # N is not unambiguous.
                  if($allel1 ne "N") {
