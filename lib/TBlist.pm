...
sub tblist {
   ...
   if(-f "$VAR_dir/$ref") {
      ...
      # parse reference .fasta to fill positions not contained in the .mpileup file (eg. uncalled or uncovered positions).
      parse_reference($logprint,$VAR_dir,$ref,$ref_hash);
      print $logprint "<INFO>\t",timer(),"\tReference genome size (bp): ",scalar(keys %$ref_hash),"!\n";
   }
   else {
      print $logprint "<ERROR>\t",timer(),"\tReference genome, $ref not found: Tblist.pm line: ", __LINE__ ," \n";
      exit 1;
   }
	# start logic...
