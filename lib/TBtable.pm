...
sub tbtable {
	...
	my $ref_hash		=	{};
	if(-f "$VAR_dir/$ref") {
		print  "<INFO>\t",timer(),"\tParsing reference genome $ref...\n";
		# Parse the reference .fasta to fill positions not contained in the .mpileup file (eg. uncalled or uncovered positions).
		parse_reference($VAR_dir,$ref,$ref_hash);
		print  "<INFO>\t",timer(),"\tReference genome size (bp): ",scalar(keys %$ref_hash),"!\n";
	}
	else {
		print  "<ERROR>\t",timer(),"\tReference genome, $ref not found! Can't continue!\n";
		exit 1;
	}
	# Start logic...
	foreach my $mpileup_file(sort { $a cmp $b } @mpileup_files) {
		...
		parse_mpile($MPILE_OUT,$POS_OUT,$output,$mpileup_file,$ref_hash,$mibqual,$lowmem,$threads);
	}
	$ref_hash		=	{};
}
